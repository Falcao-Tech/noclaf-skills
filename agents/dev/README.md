...
Novos agentes entram como `<nome>.md`. O loop de build usa: **repo-scout** (explora) → **code-executor** (implementa) → **code-reviewer** (valida pré-push) → abre PR → **pr-reviewer** (valida pós-push, decide a entrega).
